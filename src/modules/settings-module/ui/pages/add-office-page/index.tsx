...
const AddOfficePage = () => {
  ...
  const { loading, name, handleUpdate, handleSubmit } = useDepartmentOffice(api.company.createOffice);

  return (
    <ContainerLayout>
      <div className={light.pageMargin} />
      <BackUi handleClick={() => navigate("/admin")} />
      <span className={light.pageTitle}>Add Office</span>
      <div className={light.pageForm}>
        <InputUi type="text" name="Office name" value={name} handleChange={handleUpdate} />
      </div>
